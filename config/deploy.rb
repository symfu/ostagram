...
namespace :setup do
  desc 'Upload configuration files to remote server'
...
namespace :nginx do
  desc 'Create symlink in /etc/nginx/conf.d for nginx.conf of the application'
...
namespace :deploy do
  ...
  namespace :linking do
    desc 'Create symlink for images'
    task :uploads do
      on roles :all do
